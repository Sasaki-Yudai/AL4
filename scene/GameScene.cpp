...
void GameScene::Update() {
	...
	float dx = abs(player_->GetPlayerX() - enemy_->GetEnemyX());
	float dz = abs(player_->GetPlayerZ() - enemy_->GetEnemyZ());

	float playerx = player_->GetPlayerX();
	float playerz = player_->GetPlayerZ();
	float enemyx = enemy_->GetEnemyX();
	float enemyz = enemy_->GetEnemyZ();

	if (fedeoutFlag == false) {
		if (dx < 2 && dz < 2) {
			fedeoutFlag = true;
			fede = 0;
